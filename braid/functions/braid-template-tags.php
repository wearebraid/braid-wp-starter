...
if ( ! function_exists( 'braid_starter_posted_on' ) ) :
	...
	function braid_starter_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'braid-starter' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: %s: post author */
			esc_html_x( 'by %s', 'post author', 'braid-starter' ),
...
if ( ! function_exists( 'braid_starter_entry_footer' ) ) :
	...
	function braid_starter_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'braid-starter' ) );
			if ( $tags_list ) {
				/* translators: %s: tag */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'braid-starter' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'braid-starter' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
			echo '</span>';
		}
		...
		edit_post_link(
			sprintf(
				...
				esc_html__( 'Edit %s', 'braid-starter' ),
...
function braid_starter_categorized_blog() {
	$all_categories = get_transient( 'braid_starter_categories' );
	if ( false === $all_categories ) {
		...
		set_transient( 'braid_starter_categories', $all_categories );
	}
